...
from model import TemporalAttention, ResNetBlock
import argparse
import joblib
import numpy as np
import os
...
class Model(nn.Module):
    def __init__(self, in_dim, out_dim, attention_dim, lstm_hidden_dim, conv_channels):
        ...
        # self.cnn = ResNetBlock(3, 32)
        self.dropout = nn.Dropout(p=0.3)
        # LSTM for temporal feature extraction
        self.lstm = nn.LSTM(input_size=32 * 3 * 3, hidden_size=lstm_hidden_dim, batch_first=True)
        # self.lstm = nn.LSTM(input_size=6272, hidden_size=lstm_hidden_dim, batch_first=True)
        
        # Attention layer
        # self.temporal_attention = TemporalAttention(lstm_hidden_dim, attention_dim)
    ...
    def forward(self, features):
        ...
        # Temporal features
        x_out, _ = self.lstm(c_out)
        x_out = self.dropout(x_out)
        # x_out = self.temporal_attention(x_out)
    ...
    def loss(self, features, targets, weights, metrics=None):
